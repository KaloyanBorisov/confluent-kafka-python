# Protobuf Producer with Field-Level Encryption

A Kafka producer example that serializes `User` records as Protobuf and applies **field-level encryption** to PII-tagged fields before publishing to a topic.
...
This script demonstrates how to use `ProtobufSerializer` together with Confluent's Data Contract encryption rules to transparently encrypt sensitive fields in Kafka messages. The schema is derived directly from the Protobuf descriptor and registered in Schema Registry with the encryption rule attached.
...
### 2. Schema derived from Protobuf descriptor

Unlike Avro or JSON, no schema string is manually defined. The schema is extracted directly from the compiled Protobuf descriptor:

```python
_schema_to_str(user_pb2.User.DESCRIPTOR.file)
```

The `name` field in the `.proto` definition is tagged with `[(confluent.field_meta).tags = "PII"]`, marking it for encryption.
...
The schema, with the encryption rule attached, is registered in Schema Registry under `<topic>-value`. The serializer is configured with `use.latest.version: True` and `use.deprecated.format: False`.
...
```bash
python protobuf_producer_encryption.py \
  -b <bootstrap-servers> \
...
- `protobuf_consumer_encryption.py` — consumes and decrypts the messages produced by this script
- `protobuf_producer.py` — same producer without encryption
